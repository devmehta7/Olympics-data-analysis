...
if user_menu == 'Overall Analysis':
    ...
    nations_over_time = helper.data_over_years(df, "region")
    fig = px.line(nations_over_time, x="Edition", y="region")
    st.title("Participating Nations over the years")
    st.plotly_chart(fig)

    events_over_time = helper.data_over_years(df, "Event")
    fig = px.line(events_over_time, x="Edition", y="Event")
    st.title("Events over the years")
    st.plotly_chart(fig)

    athletes_over_time = helper.data_over_years(df, "Name")
...
if user_menu == 'Country-wise Analysis':
    ...
    pt = helper.country_wise_medal_heatmap(df, selected_country)
    fig, ax = plt.subplots(figsize=(20, 20))
    sns.heatmap(pt, annot=True)
    st.pyplot(fig)

    st.title("Top 10 Athletes of " + selected_country)
    top10_df = helper.most_successful_by_country(df, selected_country)
    st.table(top10_df)
...
if user_menu == 'Athlete-wise Analysis':
    ...
    fig = ff.create_distplot([x1, x2, x3, x4], [
                             'Age Distribution', 'Gold', 'Silver', 'Bronze'], show_hist=False, show_rug=False)
