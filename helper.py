...
def most_successful_by_country(df, region):
...
def country_wise_medal_heatmap(df, country):
    temp_df = df.dropna(subset=['Medal'])
    # droping redundant medal for team sport.
    temp_df.drop_duplicates(
        ['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
    pt = new_df.pivot_table(index='Sport', columns='Year', values='Medal', aggfunc='count').fillna(0).astype(
        int)
    return pt
